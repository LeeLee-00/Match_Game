...
// appendNewCardTest("card-container");
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i = 0; i <= 11; i++) {
    ...
    let image = shuffledImageClasses[i];
...
function doCardsMatch(cardObject1, cardObject2) {
  if (cardObject1.imageClass == cardObject2.imageClass) { return true; }
  else { return false; }
}
// doCardsMatchTest();
...
function onCardFlipped(newlyFlippedCard) {
  ...
  let flipUI = document.getElementById("flip-count");
  incrementCounter("flipCount", flipUI);
